...
{
    struct struct_libmqttlink_struct *g_libmqttlink_struct_ptr = &g_libmqttlink_struct;
    uint8_t number_of_notification_structer = g_libmqttlink_struct_ptr->number_of_notification_structer;
    for (uint8_t i = 0; i < number_of_notification_structer; i++)
    {
        if (!strcmp(g_libmqttlink_struct_ptr->notification_structer_ptr[i].subject, mosquitto_message_structer_ptr->topic))
        {
            g_libmqttlink_struct_ptr->notification_structer_ptr[i].notification_function_ptr(
...
static void mosq_connection_info_arrived_notification_function(struct mosquitto *notification_structer_ptr, void *obj, int result)
{
    ...
    if (result == 0)
    {
        g_libmqttlink_struct_ptr->connection_state_flag = e_libmqttlink_connection_state_connection_true;
...
static int subscribe_all_topic(void)
{
    ...
    while (1)
    {
        for (int i = 0; i < g_libmqttlink_struct_ptr->number_of_notification_structer; ++i)
        {
            int *mid = NULL;
            int qos = 2;
            int result = mosquitto_subscribe(
                g_libmqttlink_struct_ptr->mosquitto_structer_ptr,
                mid,
                g_libmqttlink_struct_ptr->notification_structer_ptr[i].subject,
                qos
            );
            ...
            {
                printf("%s(): Subscribed to topic [%s].\n", __func__, g_libmqttlink_struct_ptr->notification_structer_ptr[i].subject);
                subs_counter++;
            }
        }
        if (subs_counter == g_libmqttlink_struct_ptr->number_of_notification_structer)
            return 0;
        subs_counter = 0;
...
static int unsubscribe_all_topic(void)
{
    ...
    while (1)
    {
        ...
        if (unsubs_counter == g_libmqttlink_struct_ptr->number_of_notification_structer)
            return 0;
        unsubs_counter = 0;
        libmqttlink_sleep_milisec(1000);
    }
...
static void* mosq_connection_state_flow_function(void *login_info_ptr)
{
    ...
    void *obj = NULL;
    g_libmqttlink_struct_ptr->mosquitto_structer_ptr = mosquitto_new(id, clean_session, obj);
    if (g_libmqttlink_struct_ptr->mosquitto_structer_ptr == NULL)
    {
        printf("%s(): Failed to start Mosquitto library. Memory error.\n", __func__);
        pthread_exit(NULL);
    }

    mosquitto_username_pw_set(g_libmqttlink_struct_ptr->mosquitto_structer_ptr, g_libmqttlink_struct_ptr->user_name, g_libmqttlink_struct_ptr->password);
    mosquitto_connect_callback_set(g_libmqttlink_struct_ptr->mosquitto_structer_ptr, mosq_connection_info_arrived_notification_function);
    mosquitto_message_callback_set(g_libmqttlink_struct_ptr->mosquitto_structer_ptr, mosq_message_received_notification_function);

    int keepalive = 60;
    mosquitto_connect(g_libmqttlink_struct_ptr->mosquitto_structer_ptr, g_libmqttlink_struct_ptr->server_ip_address, g_libmqttlink_struct_ptr->server_port, keepalive);
    ...
    while (1)
    {
        int result = mosquitto_loop(g_libmqttlink_struct_ptr->mosquitto_structer_ptr, timeout, max_packets);
        if (result != MOSQ_ERR_SUCCESS)
        {
            g_libmqttlink_struct_ptr->connection_state_flag = e_libmqttlink_connection_state_connection_false;
            printf("%s(): mosquitto_loop(): Connection lost. Reason: [%s]\n", __func__, mosquitto_strerror(result));
            libmqttlink_sleep_milisec(1000);
            mosquitto_reconnect(g_libmqttlink_struct_ptr->mosquitto_structer_ptr);
        }
...
{
    struct struct_libmqttlink_struct *g_libmqttlink_struct_ptr = &g_libmqttlink_struct;
    if (g_libmqttlink_struct_ptr->notification_structer_ptr != NULL)
    {
        printf("%s(): MQTT link already active.\n", __func__);
        return -1;
    }

    g_libmqttlink_struct_ptr->server_ip_address = server_ip_address;
    g_libmqttlink_struct_ptr->server_port = server_port;
    g_libmqttlink_struct_ptr->user_name = user_name;
    g_libmqttlink_struct_ptr->password = password;
    ...
    int result = pthread_create(&g_libmqttlink_struct_ptr->link_control_thread_id, NULL, mosq_connection_state_flow_function, NULL);
    if (result)
    {
        printf("%s(): Thread could not be created. Reason: [%s]\n", __func__, strerror(result));
        return -1;
    }
    printf("%s(): Thread created. id: [%ld]\n", __func__, g_libmqttlink_struct_ptr->link_control_thread_id);
    return 0;
}
...
int libmqttlink_subscribe_mosq_subject(const char *subject, void (*notification_function_ptr)(const char *message_content, const char *subject))
{
    ...
    struct struct_libmqttlink_struct *g_libmqttlink_struct_ptr = &g_libmqttlink_struct;
    ++g_libmqttlink_struct_ptr->number_of_notification_structer;
    g_libmqttlink_struct_ptr->notification_structer_ptr = (struct struct_notification_structer *)realloc(
        g_libmqttlink_struct_ptr->notification_structer_ptr,
        g_libmqttlink_struct_ptr->number_of_notification_structer * sizeof(struct struct_notification_structer)
    );
    if (g_libmqttlink_struct_ptr->notification_structer_ptr == NULL)
    {
        printf("%s(): realloc() error. Number of subscribers: [%d]\n", __func__, g_libmqttlink_struct_ptr->number_of_notification_structer);
    }

    int idx = g_libmqttlink_struct_ptr->number_of_notification_structer - 1;
    strcpy(g_libmqttlink_struct_ptr->notification_structer_ptr[idx].subject, subject);
    g_libmqttlink_struct_ptr->notification_structer_ptr[idx].notification_function_ptr = notification_function_ptr;
...
void libmqttlink_finished_mosq(void)
{
    ...
    if (g_libmqttlink_struct_ptr->mosquitto_structer_ptr != NULL)
    {
        ...
        pthread_cancel(g_libmqttlink_struct_ptr->link_control_thread_id);

        printf("%s(): Disconnecting from Mosquitto server.\n", __func__);
        if (g_libmqttlink_struct_ptr->connection_state_flag == e_libmqttlink_connection_state_connection_true)
        {
            mosquitto_disconnect(g_libmqttlink_struct_ptr->mosquitto_structer_ptr);
        }
        mosquitto_destroy(g_libmqttlink_struct_ptr->mosquitto_structer_ptr);
        mosquitto_lib_cleanup();

        if (g_libmqttlink_struct_ptr->notification_structer_ptr != NULL)
        {
            printf("%s(): Freeing subscriber memory.\n", __func__);
            free(g_libmqttlink_struct_ptr->notification_structer_ptr);
        }

        g_libmqttlink_struct_ptr->notification_structer_ptr = NULL;
        g_libmqttlink_struct_ptr->mosquitto_structer_ptr = NULL;
        g_libmqttlink_struct_ptr->connection_state_flag = e_libmqttlink_connection_state_connection_false;
    }
 ...
 */
int libmqttlink_send_mosq_message(const char *subject, const char *message_content, enum _enum_libmqttlink_message_storage_flag_state message_storage_flag)
{
    struct struct_libmqttlink_struct *g_libmqttlink_struct_ptr = &g_libmqttlink_struct;
    if (g_libmqttlink_struct_ptr->connection_state_flag == e_libmqttlink_connection_state_connection_false)
    {
